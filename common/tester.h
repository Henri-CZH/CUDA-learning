...
#ifdef _WIN32
#include <windows.h>
void precise_sleep_us(int us) {
    LARGE_INTEGER freq, start, end;
    QueryPerformanceFrequency(&freq);
    QueryPerformanceCounter(&start);
    double target = (double)us / 1e6;
    do {
        QueryPerformanceCounter(&end);
    } while ((end.QuadPart - start.QuadPart) * 1.0 / freq.QuadPart < target);
}
#endif
...
public:
    ...
    void evaluate(Func &&hgemm, const std::string &name) {
        ...
        precise_sleep_us(m_sleep_duration * 1000); // windows, microseconds
        // usleep(m_sleep_duration * 1000); // linux, microseconds
